...
<div class="app-content">
    ...
    <div class="row ms-auto me-auto justify-content-center w-auto mt-2">
        <div class="col-md-11 p-2">
            <div class="card shadow-lg border-0 rounded-lg">
                <div class="card-body">
                    <form id="chat-form" action="<?= base_url('chat/kirim_pesan_siswa'); ?>" method="POST" enctype="multipart/form-data">
                        <div class="row g-2">
                            <div class="col-md-10">
    ...
    fetch(`<?= base_url('chat/get_messages_by_siswa'); ?>?id_pengaduan=${id_pengaduan}`)
        ...
        .then(data => {
            if (data.status === 'success') {
                ...
                data.data.forEach(t => {
                    const isSiswa = t.sender_type === 'siswa';
                    const chatClass = isSiswa ? 'justify-content-end' : 'justify-content-start';
                    const cardClass = isSiswa ? 'text-bg-primary' : 'text-bg-secondary';
...
document.getElementById('kirim').addEventListener('click', function() {
    ...
    .then(data => {
        if (data.status === 'success') {
            ...
            const isScrolledToBottom = chatContent.scrollHeight - chatContent.scrollTop <= chatContent.clientHeight + 10;
